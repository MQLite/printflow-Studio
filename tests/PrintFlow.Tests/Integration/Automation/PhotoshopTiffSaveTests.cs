...
public sealed class PhotoshopTiffSaveTests : IDisposable
{
    ...
    /// <summary>
    /// SCRUM-11101-A Gate D. A document whose W1 was retained from the customer's own file, rather
    /// than generated by the signed Action, reaches the same one save contract.
    /// </summary>
    /// <remarks>
    /// The Action is not invoked anywhere in this test, and the prepared document does not pretend
    /// it was: its provenance is <see cref="PhotoshopWhiteInkProvenance.Retained"/>, naming the
    /// managed carrier the W1 arrived on. Everything the saver actually checks — CMYK/8, four
    /// components, one non-empty W1 spot channel, 300 PPI, geometry, the untouched backing file —
    /// is identical to the generated case, because none of it was ever about the Action.
    /// </remarks>
    [Fact]
    public async Task Retained_white_ink_reaches_the_same_save_contract_without_the_Action()
    {
        Harness h = CreateHarness();
        PhotoshopW1PreparedDocument retained = h.Prepared with
        {
            Provenance = new PhotoshopWhiteInkProvenance.Retained(
                h.DocumentPath, h.Prepared.BackingWorkingSha256),
        };

        OperationResult<PhotoshopValidatedTiffCandidate> result = await h.Saver
            .SaveProductionTiffAsync(h.Opened, retained, h.Output, CancellationToken.None);
    ...
    /// <summary>
    /// Provenance is a claim the saver checks, not a label it accepts on sight.
    /// </summary>
    /// <remarks>
    /// The generalisation would be worthless if any origin were as good as none. A retained
    /// provenance that names no carrier, and a generated one that names no action, are both
    /// refused before the single native save call is reached.
    /// </remarks>
    [Theory]
    [InlineData("retained-without-a-carrier")]
    [InlineData("generated-without-an-action")]
    public async Task White_ink_with_an_unestablished_origin_is_refused_before_any_save(string shape)
    {
        Harness h = CreateHarness();
        PhotoshopWhiteInkProvenance provenance = shape == "retained-without-a-carrier"
            ? new PhotoshopWhiteInkProvenance.Retained("   ", h.Prepared.BackingWorkingSha256)
            : new PhotoshopWhiteInkProvenance.Generated(WhiteUnderbaseBranch.W1_2px, "PrintFlow DTF", "");

        OperationResult<PhotoshopValidatedTiffCandidate> result = await h.Saver
            .SaveProductionTiffAsync(
                h.Opened, h.Prepared with { Provenance = provenance }, h.Output, CancellationToken.None);
    ...
    private Harness CreateHarness(WhiteUnderbaseBranch branch = WhiteUnderbaseBranch.W1_1px)
    {
        ...
        PhotoshopW1PreparedDocument prepared = new(
            ...
            Provenance: new PhotoshopWhiteInkProvenance.Generated(
                branch, "PrintFlow DTF", branch.ToString()),
            OtherDocumentsMayBeOpen: true,
